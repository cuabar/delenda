...
//Spec Ops Recruit Training
$getallrecruits = mysql_query("SELECT * FROM recruits") or die(mysql_error());
while($recruitarray = mysql_fetch_array($getallrecruits))
{
	if($recruitarray['trainingtime'] > 0)
	{
		$recruitid = $recruitarray['id'];
		$updatetraining = mysql_query("UPDATE recruits SET skill=skill+1, trainingtime=trainingtime-1 WHERE id='$recruitid'");
	}
}

//Combat Troop Destruction
$battlezonequery=mysql_query("SELECT * FROM battlezones WHERE zoneid < 31") or die (mysql_error());
...
while($battlezonearray = mysql_fetch_array($battlezonequery))
{
	...
	if($solidaritytroops != $newsolidaritytroops)
	{
		...
		echo $soltroopslost.'<br>';
		$enterlog = mysql_query("INSERT INTO warlogs(allegiance, battlezone, type, date, time, value) VALUES('solidarity', '$zone', 'casualties', CURDATE(), CURTIME(), '$soltroopslost')") or die(mysql_error());
	}
	if($newauthoritytroops != $authoritytroops)
	{
		if($authregen > 0)
		{
			...
			mysql_query("INSERT INTO warlogs(allegiance, battlezone, type, date, time, value, value2) VALUES('authority', '$zone', 'casualties', CURDATE(), CURTIME(), '$authtroopslost', '$authregen')") or die(mysql_error());
		}
		else
		{
			$authtroopslost = $authoritytroops - $newauthoritytroops;
			echo $authtroopslost;
			mysql_query("INSERT INTO warlogs(allegiance, battlezone, type, date, time, value) VALUES('authority', '$zone', 'casualties', CURDATE(), CURTIME(), '$authtroopslost')") or die(mysql_error());
		}
	}
